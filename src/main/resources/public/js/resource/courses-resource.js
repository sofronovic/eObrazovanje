angular.module('studentServiceApp')
	.factory('CoursesResource', ['Restangular', '_', 
		function(Restangular, _){
			...
			returnValue.getCourses = function(){
				return Restangular.all("studijskiProgrami").getList().then(function(entries){
					courses = entries;
					return courses;
				});
			};
			...
			returnValue.removeCourse = function(id){
				return Restangular.one("studijskiProgrami", id).remove().then(function(){
					_.remove(courses, {
						id: id
					});
				});
